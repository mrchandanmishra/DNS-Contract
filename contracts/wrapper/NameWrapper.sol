...
import {DotLabs} from "../registry/DotLabs.sol";
    ...
    ReverseClaimer
{
    ...
    ) ReverseClaimer(_dotlabs, msg.sender) {
        dotlabs = _dotlabs;
    ...
    function setUpgradeContract(
        ...
        if (address(upgradeContract) != address(0)) {
            ...
            dotlabs.setApprovalForAll(address(upgradeContract), false);
        }
        ...
        if (address(upgradeContract) != address(0)) {
            ...
            dotlabs.setApprovalForAll(address(upgradeContract), true);
        }
    }
    ...
    function wrapETH2LD(
        ...
        // transfer the dotlabs_ record back to the new owner (this contract)
     ...
     * @param resolver The resolver address to set on the dotlabs registry (optional).
     * @param ownerControlledFuses Initial owner-controlled fuses to set
    ...
    function renew(
        ...
        try registrar.ownerOf(tokenId) returns (address registrarOwner) {
            if (
                ...
                dotlabs.owner(node) != address(this)
            ) {
    ...
    function wrap(
        ...
        address owner = dotlabs.owner(node);

        if (
            owner != msg.sender && !dotlabs.isApprovedForAll(owner, msg.sender)
        ) {
            revert Unauthorised(node, msg.sender);
        }

        if (resolver != address(0)) {
            dotlabs.setResolver(node, resolver);
        }

        dotlabs.setOwner(node, address(this));
    ...
    function setChildFuses(
        ...
        if (owner == address(0) || dotlabs.owner(node) != address(this)) {
            revert NameIsNotWrapped();
        }
        // max expiry is set to the expiry of the parent
    ...
    function setSubnodeOwner(
        ...
        if (!_isWrapped(node)) {
            dotlabs.setSubnodeOwner(parentNode, labelhash, address(this));
    ...
    /**
     * @notice Sets records for the name in the dotlabs Registry
    ...
    {
        dotlabs.setRecord(node, address(this), resolver, ttl);
        if (owner == address(0)) {
    ...
    function _checkCanCallSetSubnodeOwner(
        ...
        if (
            ...
            (subnodeOwner == address(0) ||
                ...
                dotlabs.owner(subnode) == address(0))
        ) {
