...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  if (network.tags.legacy) {
    const contract = await deploy('LegacyENSRegistry', {
      ...
      args: [],
      log: true,
      contract: await deployments.getArtifact('DotLabsRegistry'),
    })
    ...
    await deploy('DotLabsRegistry', {
      from: deployer,
      args: [contract.address],
      log: true,
      contract: await deployments.getArtifact('DotLabsRegistryWithFallback'),
    })
  } else {
    await deploy('DotLabsRegistry', {
      from: deployer,
  ...
  if (!network.tags.use_root) {
    const registry = await ethers.getContract('DotLabsRegistry')
